...
const getStatusCodeFromPrismaError = (errorCode: string): number => {
    ...
    if (errorCode.startsWith("P3") || errorCode.startsWith("P4")) return status.INTERNAL_SERVER_ERROR;
...
export const handlePrismaClientKnownRequestError = (error: Prisma.PrismaClientKnownRequestError): TErrorResponse => {
    ...
    const errorSources: TErrorSources[] = [{ path: error.code ?? "Unknown", message: metaInfo ? `${mainMessage} | ${metaInfo}` : mainMessage }];
    if (error.meta?.cause) { errorSources.push({ path: "cause", message: String(error.meta.cause) }); }
...
export const handlePrismaClientUnknownError = (error: Prisma.PrismaClientUnknownRequestError): TErrorResponse => {
    ...
    const errorSources: TErrorSources[] = [{ path: "Unknown Prisma Error", message: mainMessage }];
    return { success: false, statusCode: status.INTERNAL_SERVER_ERROR, message: `Prisma Client Unknown Request Error: ${mainMessage}`, errorSources };
};
...
export const handlePrismaClientValidationError = (error: Prisma.PrismaClientValidationError): TErrorResponse => {
    ...
    return { success: false, statusCode: status.BAD_REQUEST, message: `Prisma Client Validation Error: ${mainMessage}`, errorSources };
};
...
export const handlerPrismaClientInitializationError = (error: Prisma.PrismaClientInitializationError): TErrorResponse => {
    ...
    const errorSources: TErrorSources[] = [{ path: error.errorCode || "Initialization Error", message: mainMessage }];
...
export const handlerPrismaClientRustPanicError = (): TErrorResponse => {
    ...
    return { success: false, statusCode: status.INTERNAL_SERVER_ERROR, message: "Prisma Client Rust Panic Error: The database engine crashed due to a fatal error.", errorSources };
};
