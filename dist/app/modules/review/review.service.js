...
const attachLikedByMeToComments = async (comments, currentUserId) => {
    if (!currentUserId || comments.length === 0) {
        return comments.map((comment) => ({
            ...comment,
            likedByMe: false,
            replies: Array.isArray(comment.replies)
                ? comment.replies.map((reply) => ({ ...reply, likedByMe: false }))
                : [],
        }));
    }
    const topLevelIds = comments.map((comment) => String(comment.id));
    const replyIds = comments.flatMap((comment) => Array.isArray(comment.replies)
        ? comment.replies.map((reply) => String(reply.id))
        : []);
...
// ==================== REVIEWS ====================
const createReview = async (userId, mediaId, payload) => {
...
const getMediaReviews = async (mediaId, queryParams, options, currentUserId) => {
    const media = await prisma.media.findUnique({ where: { id: mediaId } });
    if (!media) {
        throw new AppError(httpStatus.NOT_FOUND, "Media not found");
    }
    ...
    const normalizedStatus = requestedStatus === ReviewStatus.PENDING ||
...
const getReviewById = async (reviewId, currentUserId) => {
...
// ==================== REVIEW COMMENTS ====================
const addComment = async (userId, reviewId, content) => {
    const review = await prisma.review.findUnique({ where: { id: reviewId } });
    if (!review) {
        throw new AppError(httpStatus.NOT_FOUND, "Review not found");
    }
...
const unpublishReview = async (reviewId) => {
    const review = await prisma.review.findUnique({ where: { id: reviewId } });
    if (!review) {
        throw new AppError(httpStatus.NOT_FOUND, "Review not found");
    }
...
const getMediaStats = async (mediaId) => {
    const media = await prisma.media.findUnique({ where: { id: mediaId } });
    if (!media) {
        throw new AppError(httpStatus.NOT_FOUND, "Media not found");
    }
...
const getAllPublishedReviews = async (queryParams, currentUserId) => {
    const builder = new QueryBuilder(prisma.review, queryParams, {
        searchableFields: ["content"],
...
const getAdminComments = async (queryParams) => {
    const builder = new QueryBuilder(prisma.reviewComment, queryParams, {
...
const unpublishComment = async (commentId) => {
    const comment = await prisma.reviewComment.findUnique({ where: { id: commentId } });
    if (!comment) {
        throw new AppError(httpStatus.NOT_FOUND, "Comment not found");
    }
